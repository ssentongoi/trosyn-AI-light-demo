...
class ConnectionManager:
    ...
    async def send_to_user(
        self, user_id: str, message: Dict[str, Any], queue_if_offline: bool = True
    ):
    ...
    async def broadcast_presence(self, user_id: str, status: str):
        ...
        await self.broadcast(
            presence_msg, exclude=self.user_connections.get(user_id, set())
        )
    ...
    async def check_heartbeats(self):
        ...
        for client_id, connection in self.active_connections.items():
            ...
            ).total_seconds() > self.heartbeat_interval + self.heartbeat_timeout:
...
from app.database import engine, get_db, import_models, init_db
...
from app.api.v1.endpoints import auth, department_requests, memory, roles, testing
                ...
                try:
                    ...
                    try:
                        ...
                        await handle_websocket_message(
                            client_id=client_id, user_id=user_id, message=message
                        )
                    ...
                    if connection and connection.is_authenticated():
                        ...
                        try:
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Client {client_id} did not respond to ping"
                                )
                                break
                        except Exception as e:
...
async def handle_websocket_message(
    client_id: str, user_id: str, message: Dict[str, Any]
):
...
@app.get(
    "/",
    dependencies=[
        Depends(PermissionChecker([{"resource": "system", "action": "read"}]))
    ],
)
async def read_root():
