    ...
    def __init__(self):
        model_path = os.getenv(
            ...
            os.path.join(
                ...
                "app_data/models/gemma-3n-E4B-it-Q4_K_M.gguf",
            ),
        )
        if not os.path.exists(model_path):
            raise FileNotFoundError(
                f"Gemini 3N model not found at {model_path}. Please check LLM_MODEL_PATH."
            )
        self.llm = Llama(
            model_path=model_path, n_ctx=2048, n_threads=os.cpu_count(), verbose=False
        )
