...
class MemoryService:
    ...
    def create_interaction(
        db: Session, 
        user: User, 
        interaction_data: MemoryInteractionCreate
    ) -> MemoryInteraction:
    ...
    def get_interaction(
        ...
        return db.query(MemoryInteraction).filter(
    ...
    def get_user_interactions(
        ...
        query = db.query(MemoryInteraction).filter(
            MemoryInteraction.user_id == user.id
        )
    ...
    def create_context(
        db: Session,
        user: User,
        context_data: MemoryContextCreate
    ) -> MemoryContext:
    ...
    def get_context(
        ...
        return db.query(MemoryContext).filter(
    ...
    def create_session(
        db: Session,
        user: User,
        session_data: MemorySessionCreate
    ) -> MemorySession:
    ...
    def get_user_sessions(
        ...
        return query.order_by(MemorySession.last_activity.desc()).offset(skip).limit(limit).all()
    ...
    def get_memory_stats(
        ...
        # Get total counts
        total_interactions = db.query(func.count(MemoryInteraction.id)).filter(
            MemoryInteraction.user_id == user.id
        ).scalar() or 0
        
        total_sessions = db.query(func.count(MemorySession.id)).filter(
            MemorySession.user_id == user.id
        ).scalar() or 0
        
        total_contexts = db.query(func.count(MemoryContext.id)).filter(
            MemoryContext.user_id == user.id
        ).scalar() or 0
        
        # Get interactions by day
        interactions_by_day = {}
        day_interactions = db.query(
        ...
        type_interactions = db.query(
            MemoryInteraction.source,
            func.count(MemoryInteraction.id).label('count')
        ).filter(
            MemoryInteraction.user_id == user.id
        ).group_by(MemoryInteraction.source).all()
        ...
        storage_usage = {
            'interactions': total_interactions * 500,  # Approx 500 bytes per interaction
    ...
    def search_memories(
        ...
        if include_interactions:
            ...
            results['interactions'] = db.query(MemoryInteraction).filter(
                query_filter
            ).order_by(MemoryInteraction.updated_at.desc()).offset(skip).limit(limit).all()
        ...
        if include_contexts:
            ...
            results['contexts'] = db.query(MemoryContext).filter(
                query_filter
            ).order_by(MemoryContext.updated_at.desc()).offset(skip).limit(limit).all()
