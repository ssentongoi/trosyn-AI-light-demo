...
class MemoryService:
    ...
    async def create_interaction(
        ...
        metadata = json.loads(db_interaction.metadata_) if db_interaction.metadata_ else {}
    ...
    @staticmethod
    async def get_interaction(db: AsyncSession, interaction_id: str) -> Optional[MemoryInteractionInDB]:
        """Get a memory interaction by ID"""
        result = await db.execute(select(MemoryInteraction).where(MemoryInteraction.id == interaction_id))
    ...
    async def get_user_interactions(
        ...
        query = query.order_by(desc(MemoryInteraction.created_at)).offset(skip).limit(limit)
        ...
        return [
            MemoryInteractionInDB(
                ...
                conversation_id=interaction.conversation_id,
                source=interaction.source,
                metadata=json.loads(interaction.metadata_) if interaction.metadata_ else {},
                is_private=interaction.is_private,
                retention_days=interaction.retention_days,
    ...
    async def create_context(
        ...
        # Convert JSON string back to dict for response
        context_data = json.loads(db_context.context_data) if db_context.context_data else {}
    ...
    @staticmethod
    async def get_context(db: AsyncSession, context_id: str) -> Optional[MemoryContextInDB]:
        """Get a memory context by ID"""
        result = await db.execute(select(MemoryContext).where(MemoryContext.id == context_id))
    ...
    async def update_context(
        db: AsyncSession, 
        context_id: str, 
        update_data: MemoryContextUpdate
    ...
    @staticmethod
    async def get_session(db: AsyncSession, session_id: str) -> Optional[MemorySessionInDB]:
        """Get a memory session by ID"""
        result = await db.execute(select(MemorySession).where(MemorySession.id == session_id))
    ...
    async def get_user_sessions(
        ...
        # Apply pagination and ordering
        query = query.order_by(desc(MemorySession.last_activity)).offset(skip).limit(limit)
    ...
    async def update_session(
        db: AsyncSession, 
        session_id: str, 
        update_data: MemorySessionUpdate
    ...
    @staticmethod
    async def end_session(db: AsyncSession, session_id: str) -> Optional[MemorySessionInDB]:
    ...
    async def get_memory_stats(db: AsyncSession, user_id: str) -> MemoryStats:
        ...
        result = await db.execute(
            select(
                func.sum(func.length(MemoryInteraction.query) + func.length(MemoryInteraction.response or '')).label('interaction_size'),
