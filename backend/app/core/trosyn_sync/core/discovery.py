...
class DiscoveryService:
    ...
    def __init__(
        ...
        node_type: str,
        port: int,
        on_node_discovered: Optional[Callable[[NodeInfo], Awaitable[None]]] = None,
    ):
    ...
    async def _announce(self):
        ...
        try:
            self._transport.sendto(
                json.dumps(message).encode("utf-8"), (MULTICAST_GROUP, DISCOVERY_PORT)
            )
        except Exception as e:
    ...
    async def _handle_message(self, message: bytes, addr: str):
        ...
        try:
            ...
            if is_new:
                logger.info(
                    f"Discovered new {node_type} node: {node_id} at {addr[0]}:{port}"
                )
...
def initialize_discovery_service(
    node_type: str,
    port: int,
    on_node_discovered: Optional[Callable[[NodeInfo], Awaitable[None]]] = None,
) -> DiscoveryService:
    ...
    if discovery_service_instance is None:
        discovery_service_instance = DiscoveryService(
            node_type=node_type, port=port, on_node_discovered=on_node_discovered
        )
