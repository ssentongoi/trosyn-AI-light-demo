...
def protocol_handler():
    ...
    return ProtocolHandler(
        node_id=TEST_NODE_ID, node_name=TEST_NODE_NAME, secret_key=TEST_SECRET
    )
...
async def test_secure_communication(tcp_client, tcp_server, protocol_handler):
    ...
    message = protocol_handler.create_message(MessageType.SYNC_DATA, test_data)

    # Verify message structure before signing
    assert hasattr(message, "message_type"), "Message missing message_type"
    assert isinstance(
        message.message_type, (str, MessageType)
    ...
    with patch(
        ...
        try:
            ...
            assert (
                "message_type" in sent_message_dict
            ), "Sent message missing message_type"
            ...
            assert (
                sent_message_dict["payload"] == test_data
            ), "Sent message payload does not match test data"
...
async def test_message_signing_and_validation(protocol_handler):
    ...
    assert any(
        keyword in reason.lower() for keyword in ["signature", "replay"]
    ), f"Should detect signature mismatch or replay attack, got: {reason}"

    # Create a new message for the wrong key test
    message2 = protocol_handler.create_message(MessageType.SYNC_DATA, test_data)
...
async def test_replay_attack_prevention(protocol_handler):
    ...
    message = protocol_handler.create_message(MessageType.SYNC_DATA, {"data": "test"})
    message.sign(protocol_handler.secret_key)
...
async def test_secure_communication(tcp_client, tcp_server, protocol_handler):
    ...
    async def mock_handler(message, client_id):
        ...
        response_payload = {"status": "received", "original_id": message.message_id}
    ...
    try:
        ...
        timestamp = datetime.utcnow().isoformat()
        message = Message(
            message_type=MessageType.SYNC_DATA, payload=test_data, timestamp=timestamp
        )
        ...
        assert (
            response.message_type == MessageType.SYNC_ACK
        ), f"Unexpected response type: {response.message_type}"
        assert (
            response.payload.get("status") == "received"
        ), f"Unexpected status in response: {response.payload}"
        ...
        ), f"Message not received by server. Got {len(received_messages)} messages"
        received_message, _ = received_messages[0]

        # Verify the message data matches what we sent
        assert (
            received_message.payload == test_data
        ), f"Message data corrupted. Expected {test_data}, got {received_message.payload}"

        # Verify the message was properly signed and validated
        is_valid, reason = protocol_handler.validate_message(
            received_message, check_signature=True, check_replay=True
        )
    finally:
...
async def test_unauthenticated_access(client_config, tcp_server, protocol_handler):
    ...
    try:
        ...
        try:
            await unauth_client.send_message(message, wait_for_response=True)
            ...
            logger.warning(
                f"Expected one of {expected_msgs} in error message, but got: {error_msg}"
            )
    finally:
...
async def test_message_integrity(tcp_client, tcp_server, protocol_handler):
    ...
    message = protocol_handler.create_message(
        MessageType.SYNC_DATA, test_data, sign=True  # Ensure the message is signed
    )
    ...
    assert (
        received_message.payload == test_data
    ), "Message data corrupted during transmission"
