...
from trosyn_sync.models.document import Document, DocumentSyncStatus, DocumentVersion
...
def test_node_model(db: Session):
    ...
    sync_status = NodeSyncStatus(
        node_id=node.id, remote_node_id="test-node-2", sync_status="idle"
    )
    db.add(sync_status)
    db.commit()
    db.refresh(sync_status)
    ...
    sync_queue = SyncQueue(
        document_id=doc.id, node_id=node.id, version_id=version.id, status="pending"
    )
    db.add(sync_queue)
    db.commit()
