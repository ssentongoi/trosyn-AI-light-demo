...
from ..models.auth import AuthAuditLog, RefreshToken
...
class TokenService:
    ...
    def create_token_pair(
        ...
        token_pair = self.auth_service.create_token_pair(
            node_id=node_id, node_type=node_type, scopes=scopes, refresh_token_jti=jti
        )
        ...
        refresh_token = RefreshToken(
            ...
            + timedelta(days=7),  # Match token expiration
            client_id=client_id,
    ...
    def revoke_token(
        ...
        token = (
            self.db.query(RefreshToken)
            .filter(RefreshToken.jti == token_jti, RefreshToken.is_revoked == False)
            .first()
        )
    ...
    def get_active_tokens(
        self, node_id: str, include_revoked: bool = False
    ) -> List[RefreshToken]:
        ...
        """
        query = self.db.query(RefreshToken).filter(RefreshToken.node_id == node_id)
