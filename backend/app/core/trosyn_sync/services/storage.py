...
class StorageService:
    ...
    def store_document(
        ...
        if existing_version:
            ...
            db.commit()
            db.refresh(doc)
            logger.info(f"New document '{doc.id}' created, reusing existing content from version '{existing_version.id}'.")
    ...
    def create_version(
        ...
        if not doc:
            raise ValueError(f"Document {document_id} not found, cannot create version.")
        ...
        existing_version = db.query(DocumentVersion).filter_by(document_id=document_id, version_hash=file_hash).first()
        if existing_version:
            logger.info(f"Returning existing version {existing_version.id} for document {document_id}")
        ...
        temp_version_id = db.query(DocumentVersion).count() + 1 # A temporary unique name
