...
class SecurityManager:
    ...
    def get_ssl_context(
        self, server_side: bool = False, verify_cert: Optional[bool] = None
    ) -> ssl.SSLContext:
        ...
        if verify_cert is None:
            verify_cert = os.getenv("TROSYNC_VERIFY_SSL", "").lower() not in (
                "0",
                "false",
                "no",
                "off",
        ...
        if verify_cert:
            ...
            logger.warning(
                "SSL certificate verification is disabled. This is not secure for production use."
            )

        # Log the SSL configuration
        logger.debug(
            f"Created SSL context: server_side={server_side}, "
