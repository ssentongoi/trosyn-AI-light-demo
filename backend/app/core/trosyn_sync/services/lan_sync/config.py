...
class LANConfig:
    ...
    max_missed_heartbeats: int = (
        3  # number of missed heartbeats before considering connection dead
    )
    ...
    def from_settings(cls) -> "LANConfig":
        ...
        config = cls(
            ...
            ssl_verify_mode=getattr(
                ssl,
                settings.get("LAN_SSL_VERIFY_MODE", "CERT_REQUIRED"),
                ssl.CERT_REQUIRED,
            ),
            ssl_check_hostname=settings.get("LAN_SSL_CHECK_HOSTNAME", True),
            ssl_password=settings.get("LAN_SSL_PASSWORD"),
            capabilities=settings.get(
                "LAN_CAPABILITIES",
                {"sync": True, "file_transfer": True, "model_updates": True},
            ),
        )
