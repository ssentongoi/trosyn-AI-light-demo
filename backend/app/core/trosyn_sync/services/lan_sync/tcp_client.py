...
class TCPSyncClient:
    ...
    async def connect(self, host: str, port: int) -> bool:
        ...
        try:
            ...
            try:
                ...
                connect_kwargs = {"host": host, "port": port, "ssl": ssl_context}
                if ssl_context:
                ...
                if self.require_auth:
                    if not self.auth_callback:
                        logger.error(
                            "Authentication required but no auth_callback provided"
                        )
            ...
            if heartbeat_interval > 0:
                ...
                logger.debug(
                    f"Started heartbeat task with interval: {heartbeat_interval}s"
                )
            ...
            try:
                ...
                original_heartbeat_handler = self.message_handlers.get(
                ...
                logger.debug(
                    "Waiting for first heartbeat to confirm connection stability..."
                )
                await asyncio.wait_for(first_heartbeat.wait(), timeout=connect_timeout)

                # Restore the original handler
                if original_heartbeat_handler:
                    self.message_handlers[MessageType.HEARTBEAT] = (
                        original_heartbeat_handler
                    )
                ...
                logger.warning(
                    f"Connection to {host}:{port} did not receive initial heartbeat after {connect_timeout} seconds"
                )
    ...
    async def _process_message(self, data: bytes) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"Received {message.message_type.name} from server (ID: {message.message_id})"
                )
            ...
            if self.require_auth and not self.authenticated:
                logger.warning(
                    f"Received {message.message_type.name} before authentication. Discarding."
                )
            ...
            if self.require_auth and message.message_type not in [
                MessageType.HEARTBEAT
            ]:
                is_valid, reason = self.handler.validate_message(
                    message, check_signature=True
                )
                if not is_valid:
            ...
            if handler:
                try:
                    ...
                    logger.error(
                        f"Error in {message.message_type.name} handler: {e}",
                        exc_info=True,
                    )
                ...
                logger.warning(
                    f"No handler for message type: {message.message_type.name}"
                )
    ...
    async def _handle_error_message(self, message: Message) -> None:
        ...
        logger.error(
            f"Server error: {error_msg} (code: {error_code}, request_id: {request_id})"
        )

        # Handle specific error codes
        if error_code == "auth_required":
            self.authenticated = False
            self._notify_auth_handlers(
                False,
                {
                    "code": error_code,
                    "message": error_msg,
        ...
        if request_id and request_id in self.pending_requests:
            ...
            if not future.done():
                future.set_exception(
                    ConnectionError(f"Server error: {error_msg} (code: {error_code})")
                )
        ...
        try:
            ...
            try:
                auth_request = self.handler.create_message(
                    MessageType.AUTH_REQUEST, auth_data
                )

                # Validate the message before sending
                is_valid, reason = self.handler.validate_message(
                    auth_request, check_signature=False
                )
                if not is_valid:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.debug(
                        f"Authentication request sent (request_id: {request_id}, attempt {attempt + 1}/{max_retries})"
                    )
                    break
                except (ConnectionError, asyncio.TimeoutError) as e:
                    if attempt == max_retries - 1:
                        logger.error(
                            f"Failed to send auth request after {max_retries} attempts"
                        )
                        raise
                    logger.warning(
                        f"Auth request failed (attempt {attempt + 1}/{max_retries}): {e}"
                    )
            ...
            try:
                response = await asyncio.wait_for(
                    response_future, timeout=15.0  # Increased timeout for auth
                )
            except asyncio.TimeoutError:
    ...
    async def _handle_auth_response(self, message: Message) -> None:
        ...
        try:
            ...
            if not future:
                logger.warning(
                    f"Received auth response for unknown request_id: {request_id}"
                )
                return

            if future.done():
                logger.warning(
                    f"Received duplicate auth response for request_id: {request_id}"
                )
                return
    ...
    async def _handle_error(self, message: Message) -> None:
        ...
        logger.error(
            f"Received error from server: '{error_msg}' (request_id: {request_id})"
        )
        ...
        if request_id and request_id in self.pending_requests:
            ...
            if not future.done():
                future.set_exception(
                    ConnectionAbortedError(f"Server error: {error_msg}")
                )
        else:
    ...
    async def _reconnect(self) -> None:
        ...
        while not self.connected and attempt < max_reconnect_attempts:
            ...
            try:
                logger.info(
                    f"Attempting to reconnect (attempt {attempt}/{max_reconnect_attempts}) in {self.reconnect_delay} seconds..."
                )
                ...
                if await self.connect(self.config.sync_host, self.config.sync_port):
                    ...
                    logger.info(
                        "Successfully reconnected and re-authenticated with server"
                    )
    ...
    async def send_message(
        ...
        if (
            ...
            and message.message_type == MessageType.AUTH_REQUEST
        ):
            logger.debug(
                "Bypassing connection readiness check for authentication message"
            )
        ...
        try:
            ...
            if wait_for_response:
                ...
                logger.debug(
                    f"[CLIENT] Added request_id to message payload: {request_id}"
                )
            ...
            if not message_to_send.signature and self.handler.secret_key:
                logger.debug(
                    f"[CLIENT] Signing message {message_to_send.message_id} before sending"
                )
                logger.debug(
                    f"[CLIENT] Message before signing: {message_to_send.__dict__}"
                )
                ...
                if message_to_send.payload:
                    ...
                    for k, v in message_to_send.payload.items():
                        logger.debug(
                            f"[CLIENT]   {k}: {v!r} (type: {type(v).__name__})"
                        )

                # Sign the message
                message_to_send.sign(self.handler.secret_key)
                logger.debug(
                    f"[CLIENT] Message after signing: {message_to_send.__dict__}"
                )
                ...
                try:
                    ...
                    if not is_valid:
                        logger.error(
                            "[CLIENT] Message failed signature verification immediately after signing!"
                        )
                        logger.error(f"[CLIENT] Message: {message_to_send.__dict__}")
                        logger.error(
                            f"[CLIENT] Signing key: {self.handler.secret_key[:8].hex()}..."
                        )
                except Exception as e:
                    logger.error(
                        f"[CLIENT] Error verifying signature after signing: {e}"
                    )
                    raise
            else:
                logger.debug(
            ...
            logger.debug(
                f"[CLIENT] Message type before validation: {type(message_to_send.message_type).__name__}"
            )
            if hasattr(message_to_send.message_type, "name"):
                logger.debug(
                    f"[CLIENT] Message type name: {message_to_send.message_type.name}"
                )

            # Validate the message before sending
            logger.debug(
                f"[CLIENT] Validating message {message_to_send.message_id} before sending"
            )
            try:
                is_valid, reason = self.handler.validate_message(
                    message_to_send, check_signature=check_signature
                )
                if not is_valid:
                    logger.error(f"[CLIENT] Message validation failed: {reason}")
                    logger.error(
                        f"[CLIENT] Message details: {message_to_send.__dict__}"
                    )
                    raise ValueError(f"Invalid message: {reason}")
            except Exception as e:
                logger.error(f"[CLIENT] Error during message validation: {str(e)}")
                logger.error(
                    f"[CLIENT] Message type: {type(message_to_send.message_type).__name__}"
                )
            ...
            if wait_for_response and future:
                try:
                    ...
                    logger.warning(
                        f"Timeout waiting for response to message {request_id}"
                    )
                    raise
                finally:
    ...
    async def _send_heartbeats(self) -> None:
        ...
        heartbeat_timeout = getattr(
            self.config, "heartbeat_timeout", 10
        )  # Default to 10s if not set
        max_missed = getattr(
            self.config, "max_missed_heartbeats", 3
