...
from ..models.document import Document, DocumentSyncStatus, DocumentVersion
...
class SyncEngine:
    ...
    def __init__(
        ...
        self.local_node = (
            db.query(Node).filter(Node.node_id == self.node_id_str).first()
        )
        if not self.local_node:
            raise ValueError(
                f"SyncEngine initialization failed: Node '{self.node_id_str}' not found in the database."
            )
    ...
    async def sync_with_node(self, remote_node_id: str) -> Dict[str, Any]:
        ...
        async with self._sync_lock:
            ...
            sync_status = NodeSyncStatus.get_or_create(
                self.db, self.node_id, remote_node_id
            )

            # Get remote node info
            remote_node = (
                self.db.query(Node).filter(Node.node_id == remote_node_id).first()
            )
    ...
    async def _generate_local_manifest(self) -> Dict[str, Any]:
        ...
        docs = (
            ...
            .filter(Document.is_deleted == False)  # noqa
            .all()
        )
    ...
    def _determine_sync_actions(
        self, local_manifest: Dict[str, Any], remote_manifest: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        remote_docs = {
            doc["document_id"]: doc for doc in remote_manifest.get("documents", [])
        }
        ...
        for local_doc in local_manifest.get("documents", []):
            ...
            if doc_id not in remote_docs:
                ...
                if local_doc["version_hash"] != remote_doc["version_hash"]:
                    ...
                    local_updated = (
                        datetime.fromisoformat(local_doc["updated_at"])
                        if local_doc["updated_at"]
                        else datetime.min
                    )
                    remote_updated = (
                        datetime.fromisoformat(remote_doc["updated_at"])
                        if remote_doc["updated_at"]
                        else datetime.min
                    )
                    ...
                    has_common_ancestor = not local_ancestors.isdisjoint(
                        remote_ancestors
                    )
        ...
        local_doc_ids = {
            doc["document_id"] for doc in local_manifest.get("documents", [])
        }
    ...
    async def _execute_sync_actions(
        self, actions: List[Dict[str, Any]], remote_node: Node
            ...
            try:
                ...
                if action_type == "push":
                    ...
                    result = await self._push_document(
                        doc_id, action["version_id"], remote_node.api_url
                    )
                    ...
                    results["metadata_merged"].append(
                        {"document_id": doc_id, "merged_metadata": merged}
                    )

            except Exception as e:
                logger.error(
                    f"Failed to execute sync action {action}: {str(e)}", exc_info=True
                )
    ...
    async def _push_document(
        self, document_id: str, version_id: str, remote_base_url: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            version = next((v for v in doc.versions if str(v.id) == version_id), None)
            if not version:
                raise ValueError(
    ...
    async def _resolve_document_conflict(
        ...
        try:
            ...
            local_version = next(
                (v for v in local_doc.versions if str(v.id) == local_version_id), None
            )
            if not local_version:
                raise ValueError(
                    f"Local version {local_version_id} not found in document {doc_id}"
                )

        except Exception as e:
            logger.error(
                f"Error accessing local document {doc_id} version {local_version_id}: {str(e)}"
            )
            raise
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.debug(
                    f"Fetching remote metadata from {remote_metadata_url} (attempt {attempt + 1}/{max_retries})"
                )
                ...
                if e.response.status_code == 404:
                    raise ValueError(
                        f"Remote version {remote_version_id} not found on node {remote_node.node_id}"
                    ) from e
        ...
        if not remote_metadata:
            raise RuntimeError(
                "Failed to fetch remote metadata after multiple attempts"
            )

        # If no strategy specified, use the default from settings or 'newer_wins'
        strategy = strategy or getattr(
            self.local_node, "default_conflict_strategy", "newer_wins"
        )
        ...
        try:
            if strategy == "local_wins":
                ...
                result = await self._push_document(
                    doc_id, local_version_id, remote_node.api_url
                )
                resolution.update({"action": "pushed_local", "result": result})

            elif strategy == "remote_wins":
                # Use remote version, pull it to local
                result = await self._pull_document(
                    doc_id, remote_version_id, remote_node.api_url, remote_node.id
                )
                resolution.update({"action": "pulled_remote", "result": result})
                ...
                if local_updated > remote_updated:
                    ...
                    resolution.update(
                        {"action": "pushed_local_newer", "result": result}
                    )
                else:
                    result = await self._pull_document(
                        doc_id, remote_version_id, remote_node.api_url, remote_node.id
                    ...
                    resolution.update(
                        {"action": "pulled_remote_newer", "result": result}
                    )
                ...
                resolution.update({"action": "merged", "result": merged})
    ...
    async def _merge_document_metadata(
        ...
        for key, value in (remote_metadata or {}).items():
            ...
            if (
                ...
                and isinstance(merged_metadata[key], dict)
        ...
        return {
            ...
            "merged_metadata": merged_metadata,
            "local_metadata_keys": (
                list(local_metadata.keys()) if local_metadata else []
            ),
            "remote_metadata_keys": (
                list(remote_metadata.keys()) if remote_metadata else []
            ),
    ...
    async def _pull_document(
        ...
        try:
            ...
            metadata_response = await client.get(
                metadata_url, timeout=HTTP_REQUEST_TIMEOUT
            )
            ...
            response = await client.stream(
                "GET", download_url, timeout=SYNC_OPERATION_TIMEOUT
            )
            response.raise_for_status()
            ...
            with temp_file.open("rb") as f:
                ...
                # Download the file using our temp file utility
                async with temp_download_file(
                    f"{document_id}_{version_id}"
                ) as temp_file:
                    # Download the file content
                    download_url = f"{remote_base_url}/api/v1/sync/documents/{document_id}/versions/{version_id}/download"
