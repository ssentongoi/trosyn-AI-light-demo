...
from ...core.auth import AuthService, Token, TokenData, get_auth_service
...
async def revoke_token(
    ...
    current_user: TokenData = Depends(
        lambda: get_auth_service().verify_token(request.token, token_type="refresh")
    ),
) -> dict:
    """
    ...
    try:
        ...
        token_data = auth_service.verify_token(request.token, token_type="refresh")
...
async def list_tokens(
    current_user: TokenData = Depends(
        lambda: get_auth_service().verify_token(Depends(oauth2_scheme))
    ),
    token_service: TokenService = Depends(get_token_service),
    ...
    try:
        tokens = token_service.get_active_tokens(
            node_id=current_user.node_id, include_revoked=include_revoked
        )
...
@router.post(
    "/cleanup", status_code=status.HTTP_200_OK, summary="Clean up expired tokens"
)
