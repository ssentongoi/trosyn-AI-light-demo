...
async def process_document(
    ...
    chunk_size_form: Optional[int] = Form(
        None, description="Maximum size of each chunk in characters"
    ),
    keep_file_form: Optional[bool] = Form(
        None, description="Whether to keep the uploaded file after processing"
    ),
) -> Dict[str, Any]:
    """
    ...
    logger.debug(
        f"  - chunk_size (original): {chunk_size} (type: {type(chunk_size) if chunk_size is not None else None})"
    )
    logger.debug(
        f"  - chunk_size_form: {chunk_size_form} (type: {type(chunk_size_form) if chunk_size_form is not None else None})"
    )
    logger.debug(
        f"  - effective_chunk_size: {effective_chunk_size} (type: {type(effective_chunk_size)})"
    )
    logger.debug(
    ...
    try:
        ...
        try:
            ...
            processed_doc = request_processor.process_document(
                str(temp_file), chunk_size=effective_chunk_size
            )
...
async def search_documents(
    search_query: SearchQuery = Body(
        ..., example={"query": "sample search query", "k": 5}
    )
):
    """
