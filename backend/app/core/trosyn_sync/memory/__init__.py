...
class MemoryEngine:
    ...
    def _get_default_memory(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "name": "",
            "role": "",
            "preferences": {"voice": "neutral", "assistant_mode": "detailed"},
            "context": {"last_topics": [], "current_focus": "", "recent_documents": []},
            "usage_stats": {
                "sessions": 0,
    ...
    def update_context(self, context_updates: Dict[str, Any]) -> None:
        ...
        for key, value in context_updates.items():
            if (
                key in self.memory
                and isinstance(self.memory[key], dict)
    ...
    def add_interaction(
        ...
        # Update last active
        self.memory["usage_stats"]["last_active"] = datetime.now(
            timezone.utc
        ).isoformat()
        self.memory["usage_stats"]["sessions"] = (
            self.memory["usage_stats"].get("sessions", 0) + 1
        )
    ...
    def get_context_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "context": {
                "current_focus": self.memory.get("context", {}).get(
                    "current_focus", ""
                ),
                "recent_topics": self.memory.get("context", {}).get("last_topics", [])[
                    :3
                ],
                "recent_documents": self.memory.get("context", {}).get(
                    "recent_documents", []
                )[:3],
            },
    ...
    @classmethod
    def import_memory(
        cls, user_id: str, import_path: Path, encryption_key: Optional[bytes] = None
    ) -> "MemoryEngine":
        """
