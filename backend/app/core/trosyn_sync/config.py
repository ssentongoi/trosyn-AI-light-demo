...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = os.getenv(
        "DATABASE_URL", f"sqlite:///{STORAGE_ROOT}/trosyn_sync.db"
    )
    ...
    MEMORY_MAX_SIZE_MB: int = int(
        os.getenv("MEMORY_MAX_SIZE_MB", "10")
    )  # Max memory file size in MB
    MEMORY_MAX_INTERACTIONS: int = int(
        os.getenv("MEMORY_MAX_INTERACTIONS", "1000")
    )  # Max number of interactions to keep
    ...
    DISCOVERY_MULTICAST_GROUP: str = os.getenv(
    ...
    @field_validator(
        "STORAGE_ROOT", "DOCUMENTS_DIR", "VERSIONS_DIR", "TMP_DIR", mode="before"
    )
    @classmethod
