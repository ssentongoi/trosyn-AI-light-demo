...
from ....schemas.department_request import DepartmentRequest as DepartmentRequestSchema
from ....schemas.department_request import (
    DepartmentRequestCreate,
    DepartmentRequestListResponse,
    DepartmentRequestUpdate,
)
from ....schemas.department_request import RequestAttachment as RequestAttachmentSchema
from ....schemas.department_request import (
...
from ....services.department_request import (
    DepartmentRequestService,
    get_department_request_service,
)
...
# Request endpoints
@router.post(
    "/", response_model=DepartmentRequestSchema, status_code=status.HTTP_201_CREATED
)
async def create_request(
    ...
    db_request = await service.create_request(
        request_data=request_data, user_id=current_user.id
    )
...
async def update_request(
    ...
    try:
        ...
        updated_request = await service.update_request(
            request_id=request_id, request_data=request_data, user_id=current_user.id
        )
...
async def delete_request(
    ...
    try:
        ...
        await service.delete_request(request_id=request_id, user_id=current_user.id)
...
async def reject_request(
    ...
    try:
        ...
        db_request = await service.reject_request(
            request_id=request_id, user_id=current_user.id, reason=reason
        )
        return db_request
    except HTTPException as e:
        raise e
...
async def add_comment(
    ...
    try:
        ...
        db_comment = await service.add_comment(
            request_id=request_id, comment_data=comment_data, user_id=current_user.id
        )
...
@router.post(
    ...
    response_model=RequestAttachmentSchema,
...
async def upload_attachment(
    ...
    try:
        db_attachment = await service.add_attachment(
            request_id=request_id, file_data=file, user_id=current_user.id
        )
