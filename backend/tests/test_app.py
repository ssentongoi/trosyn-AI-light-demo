...
from app.api.v1.endpoints.department_requests import (
    router as department_requests_router,
)
...
async def override_get_current_user() -> User:
    ...
    async with AsyncSession(engine) as session:
        ...
        result = await session.execute(select(Company).where(Company.name == "TestCo"))
