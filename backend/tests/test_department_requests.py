...
async def test_minimal_create_department_request(
    async_client: AsyncClient, db_session: AsyncSession, test_data: Dict[str, Any]
):
    ...
    response = await async_client.post(
        "/api/v1/department-requests/", json=request_data
    )
    ...
    ), f"Failed to create department request. Response: {response.text}"
    ...
    assert (
        department_request["title"] == request_data["title"]
    ), "Title doesn't match input"
    assert (
        department_request["description"] == request_data["description"]
    ), "Description doesn't match input"
    assert (
        department_request["request_type"] == request_data["request_type"]
    ), "Request type doesn't match input"
    assert (
        department_request["priority"] == request_data["priority"]
    ), "Priority doesn't match input"
    assert (
        department_request["status"] == "draft"
    ), "New department request should have draft status"  # Note: status is lowercase in schema
