...
def test_load_document_invalid_type(document_service):
    result = document_service.load_document(file_content="Test content", file_type="pdf")
    assert result["status"] == "error"
    assert "error" in result
...
def test_redact_text(document_service):
    ...
    result = document_service.redact_text(text=test_text, sensitive_terms=["123-45-6789"])
    assert result["status"] == "success"
