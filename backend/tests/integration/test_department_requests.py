...
async def test_create_department_request(
    ...
    department = (
        await test_department
        if hasattr(test_department, "__await__")
        else test_department
    )
    headers = (
        await test_auth_headers
        if hasattr(test_auth_headers, "__await__")
        else test_auth_headers
    )
...
async def test_get_department_request(
    ...
    # Ensure all async fixtures are awaited
    request = (
        await test_department_request
        if hasattr(test_department_request, "__await__")
        else test_department_request
    )
    headers = (
        await test_auth_headers
        if hasattr(test_auth_headers, "__await__")
    ...
    response = await client.get(
        f"/api/v1/department-requests/{request.id}", headers=headers
    )
...
async def test_list_department_requests(
    ...
    response = await client.get("/api/v1/department-requests/", headers=headers)
...
async def test_update_department_request(
    ...
    response = await client.put(
        f"/api/v1/department-requests/{request.id}", json=update_data, headers=headers
    )
...
async def test_add_comment_to_request(
    ...
    comment_data = {"content": "This is a test comment", "is_internal": False}
