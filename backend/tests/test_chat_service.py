...
async def test_get_conversation_with_messages(db_session: AsyncSession):
    ...
    message1 = ChatMessage(conversation_id=conversation.id, user_id=test_user.id, role=MessageRole.USER, content="Hello")
    message2 = ChatMessage(conversation_id=conversation.id, user_id=test_user.id, role=MessageRole.ASSISTANT, content="Hi there!")
