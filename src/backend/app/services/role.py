...
from app.schemas.role import (
    RoleCreate, RoleUpdate, PermissionCreate, PermissionInDB, RoleInDB,
...
class RoleService:
    ...
    @staticmethod
    async def get_roles(db: AsyncSession, skip: int = 0, limit: int = 100) -> List[RoleInDB]:
        result = await db.execute(select(Role).offset(skip).limit(limit))
        roles = result.scalars().all()
        return [RoleInDB.from_orm(role) for role in roles]

    @staticmethod
    async def update_role(db: AsyncSession, role_id: int, role: RoleUpdate) -> Optional[RoleInDB]:
        result = await db.execute(select(Role).filter(Role.id == role_id))
        db_role = result.scalars().first()
        if not db_role:
            return None
...
class PermissionService:
    @staticmethod
    async def create_permission(db: AsyncSession, permission: PermissionCreate) -> PermissionInDB:
    ...
    @staticmethod
    async def get_permission_by_name(db: AsyncSession, name: str) -> Optional[PermissionInDB]:
