...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('session_id', sa.String(36), nullable=True),  # We'll add the FK after creating memory_sessions
        sa.Column('created_at', sa.DateTime, server_default=sa.func.now(), nullable=False),
        sa.Column('updated_at', sa.DateTime, server_default=sa.func.now(), nullable=False),
    )
    
    # Create indexes for memory_interactions
    op.create_index('ix_memory_interactions_user_id', 'memory_interactions', ['user_id'])
    op.create_index('ix_memory_interactions_created_at', 'memory_interactions', ['created_at'])
    op.create_index('ix_memory_interactions_updated_at', 'memory_interactions', ['updated_at'])
    ...
    op.create_table(
        ...
        sa.Column('description', sa.Text, nullable=True),
        sa.Column('tags', sa.Text, nullable=True, server_default='[]'),  # Store as JSON string
    ...
    op.create_index('ix_memory_sessions_last_activity', 'memory_sessions', ['last_activity'])
    
    # Create index for session_id in memory_interactions
    op.create_index('ix_memory_interactions_session_id', 'memory_interactions', ['session_id'])
