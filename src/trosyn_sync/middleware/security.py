...
class CSRFMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, secret_key: Optional[str] = None):
        super().__init__(app)
        self.secret_key = secret_key or str(time.time())  # Use a secure key in production
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        # Debug logging
        if is_test_env:
            logger.debug(f"CSRF protection skipped in test environment. Signals: "
        ...
        if not is_test_env and request.method in ["POST", "PUT", "PATCH", "DELETE"]:
            ...
            is_api_request = request.url.path.startswith("/api/") and request.headers.get("Authorization")
            
            # Validate token if not an authenticated API request
            if not is_api_request and (not csrf_token or not cookie_token or csrf_token != cookie_token):
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, limit: int = 100, window: int = 60, cache: Optional[Callable] = None):
        super().__init__(app)
        self.limit = limit
        self.window = window
        self.rate_limits = {}
...
def setup_security_middleware(app: FastAPI) -> None:
    ...
    if getattr(settings, "ENVIRONMENT", "development") == "production" and getattr(settings, "ENABLE_HSTS", True):
