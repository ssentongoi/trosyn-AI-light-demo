...
class TaskManager:
    ...
    async def start(self) -> None:
        ...
        self._tasks["periodic_sync"] = asyncio.create_task(
        ...
        while self._running:
            try:
                ...
                try:
                    ...
                    nodes_to_sync = db.query(Node).join(
                        NodeSyncStatus,
                    ...
                    for node in nodes_to_sync:
                        try:
                            logger.info(f"Syncing with node {node.node_id} ({node.display_name})")
                ...
                logger.debug(f"Sync cycle completed in {elapsed:.2f}s, "
    ...
    async def trigger_immediate_sync(self, node_id: Optional[str] = None) -> Dict[str, Any]:
