...
class TCPSyncClient:
    ...
    def __init__(
        ...
        self._handling_connection_error = False  # Flag to prevent reentrant error handling
        
        # Tasks and timers
        self.reconnect_task: Optional[asyncio.Task] = None
        self.heartbeat_task: Optional[asyncio.Task] = None
        self.receive_task: Optional[asyncio.Task] = None
        self.reconnect_delay = 2  # Initial reconnect delay in seconds (reduced from 5s for tests)
    ...
    async def connect(self, host: str, port: int, timeout: Optional[float] = None) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.reader, self.writer = await asyncio.wait_for(connect_coro, timeout=connect_timeout)
    ...
    async def _authenticate(self) -> bool:
        ...
        try:
            ...
            auth_message = Message(
                ...
                source={"node_id": self.config.node_id, "node_name": self.config.node_name},
                payload=auth_data
            )
            ...
            try:
                ...
                if auth_response.get('status') == 'success':
                    ...
                    logger.debug("Connection readiness event set after successful authentication")
        ...
        if request_id and request_id in self.pending_requests:
            ...
            if not future.done():
                if message.payload.get('status') == 'success':
                    ...
                    # Set the connected event to indicate full readiness
                    self._connected_event.set()
                    logger.debug("Connection readiness event set after successful authentication response")
                    future.set_result(message)
                else:
                    error = ConnectionRefusedError(message.payload.get('message', 'Authentication failed'))
                    future.set_exception(error)

    async def _handle_error(self, message: Message):
        """Handle error messages from the server."""
        error_payload = message.payload or {}
        error_message = error_payload.get('error', error_payload.get('message', 'Unknown error'))
        error_code = error_payload.get('code', 'UNKNOWN')
        logger.error(f"Received error from server: {error_message} (Code: {error_code})")
    ...
    async def _handle_connection_error(self) -> None:
        ...
        if hasattr(self, '_handling_connection_error') and self._handling_connection_error:
    ...
    def _reconnect(self) -> None:
        ...
        async def reconnect_task() -> None:
            ...
            while not self.connected and attempt < max_attempts:
                ...
                logger.info(f"Reconnection attempt {attempt}/{max_attempts} in {current_delay}s")
                await asyncio.sleep(current_delay)
                
                if hasattr(self, '_last_host') and hasattr(self, '_last_port'):
                    try:
                        logger.info(f"Trying to connect to {self._last_host}:{self._last_port}")
                        ...
                        if self.connected:
                            ...
                            logger.warning("Connection attempt did not result in a connected state")
                        ...
                        if attempt >= max_attempts:
                            logger.error("Max reconnection attempts reached due to unexpected error.")
                            self._notify_connection_handlers(False)
                else:
                    logger.error("Cannot reconnect: No previous connection details available")
                    self._notify_connection_handlers(False)
                    break
    ...
    async def send_message(self, message: Message) -> None:
        ...
        if message.message_type != MessageType.AUTH_REQUEST and not self._connected_event.is_set():
            try:
                await asyncio.wait_for(
    ...
    def _notify_auth_handlers(self, authenticated: bool, auth_data: Optional[Dict[str, Any]] = None):
